...
with col1:
    ...
    sex = st.selectbox("Sex", options=[0, 1], format_func=lambda x: "男性 (0)" if x == 0 else "女性 (1)")
...
if st.button("🔮 予測実行", type="primary", use_container_width=True):
    ...
    if r0['vault'] is not None:
        ...
        ax_v.set_title('Vault Gauge  (solid=recommended, dashed=-0.25, dotted=-0.50)', fontweight='bold')
